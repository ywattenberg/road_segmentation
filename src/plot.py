...
def plot_model_comp(csv_dir="model-comparison"):
    ...
    fig.savefig(f"figures/model-comparison-10.png", dpi=300)
...
def plot_encoder_comp_unetpp(csv_dir="encoder-comparison"):
    ...
    for file in sorted(os.listdir(csv_dir)):
        ...
        if file.split("-")[1] != "efficientnet":
            label = f"{file.split('-')[1]}"
        else:
            label = f"{file.split('-')[1]}-{file.split('-')[2]}"
    ...
    plt.legend()
    plt.tight_layout(pad=3)
    plt.title("Unet++ model with different encoders")
    plt.xlabel("Epoch")
    plt.ylabel("Dice Loss")
    fig.savefig(f"figures/encoder-comparison-unetpp-30.png", dpi=300)
...
def plot_model_encoder_comp(csv_dir="encoder-comparison"):
    ...
    for file in sorted(os.listdir(csv_dir)):
        ...
        if file.split("-")[1] != "efficientnet":
            ...
            label = f"{model_name}, {file.split('-')[1]}-{file.split('-')[2]}"
        plt.plot(
            df["epoch"],
            df["DiceLoss"],
            label=label,
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Dice Loss")
    fig.savefig(f"figures/model-encoder-comparison-30.png", dpi=300)
...
def plot_final_models(csv_dir="final-models"):
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Dice Loss")
    fig.savefig(f"figures/final-models.png", dpi=300)
