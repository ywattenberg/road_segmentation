...
if __name__ == "__main__":
    ...
    dataset = torchvision.datasets.EuroSAT(
        root="data/", download=False, transform=transform
    )
    print(dataset)
    ...
    model.load_state_dict(
        torch.load("best_model_weights_feature_extractor_transformer.pth")
    )
    ...
    trainer = Trainer(
        ...
        batch_size=128,
        epochs=80,
        test_metrics=[loss_fn],
        test_metric_names=["BinaryCrossEntropy"],
        epochs_between_safe=10,
        name="feature_extractor_transformer",
    )
