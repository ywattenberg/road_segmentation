...
from trainer_dice import Trainer
...
def main(
    ...
    dataset = ETHDataset(
        image_path, mask_path, augment_images=True
    )
    ...
    trainer = Trainer(
        ...
        name=f"{model_name}-{encoder_name}-{encoder_weights}-Dice",
    )

    scores = trainer.train_test()
    scores.to_csv(
        f"smp/{model_name}-{encoder_name}-{encoder_weights}-Dice.csv", index=False
    )
